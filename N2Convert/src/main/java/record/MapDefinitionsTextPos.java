...
/**
 * Procesamiento de las definiciones del fichero JSON para poder interpretar los
 * ficheros de texto delimitados por posiciones
 */
public class MapDefinitionsTextPos {

	/**
	 * Lee el JSON de definiciones y construye el mapa indexado por tipos. Hecho
	 * público para que cualquier módulo (Procesamiento o Conversión) pueda
	 * inicializar sus layouts.
	 */
		...
		try {
			...
			if (records.isArray()) {
				for (JsonNode record : records) {
					...
					// Si no viene tipo en el JSON, usamos "default"
	...
	/**
	 * Analiza una línea física y devuelve el tipo correspondiente.
	 */
	public static String obtenerTipoDeLinea(String linea, Map<String, RecordDefinitionTextPos> mapaDefiniciones) {
		// 1. Si el mapa tiene una única estructura, evitamos el substring (Eficiencia
		// máxima)

...
/**
 * 
 * Clase en la que tendremos la tareas así como su configuración y los valores
 * que se establecieron por la consola
 * 
 * 
 * 
 */
public class TaskOptionsConfig {
	...
	/**
	 * Configuración de la tarea, concretamente la info de configuracion del fichero
	 * config.json
	 */
	 ...
	 * 
	 * @param fctc   - valores del fichero de configuración del proyecto y sus
	 *               tareas
	...
	/**
	 * Caramos los parametros suministrado en formato en var1=val1 en un HashMap
	 * indexado por nombre de parámetro
	 * 
	 * @param params
	 * @return
	 */
	...
	private Object convertType(String cmdVal, ConfigParam detail) {
		...
		case ListInteger:
			...
			for (int i = 0; i < temp.size(); i++) {
				...
				// ApachePOI trabaja inicia con 0, pero excel no. Cómo por comandos pedimos que
				// inicie por 1, en la carga restamos el desfase
				if (detail.isIndex()) {
					result[i] = result[i] - 1;
				}
			}
	...
	/**
	 * Carga la lista de opciones según la configuracion del json y los parámetros
	 * recibido como comandos para la tarea los valida y carga
	 * 
...
//			System.err.println("Parametros: " + this.configTask.getParams());
			...
			// recorre los parámetros específicos de la tarea, los carga convertidos y si
			// falta alguno obligatorio lanza una excepción.
			this.configTask.getParams().forEach((name, detail) -> {
				if (this.commandParams.containsKey(name)) {
					...
					// System.err.println("Valor convertido para " + name + ": " +
					// finalValue.toString());
